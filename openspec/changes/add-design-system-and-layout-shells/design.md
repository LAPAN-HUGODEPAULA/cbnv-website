...
- Traduzir os tokens de cor e tipografia do Stitch para o `tailwind.config.js`.
...
### D3: Tailwind CSS v4 para Tokens

**Decisão:** Definir a paleta oficial no `tailwind.config.js` usando nomes semânticos (`brand-primary`, `brand-secondary`, `surface-deep`, etc.) baseados no Stitch.

**Racional:** Garante que a interface seja consistente e que mudanças na cor da marca possam ser aplicadas globalmente apenas alterando o config.
...
- **[Performance de CSS]** → Mitigação: O build do Tailwind 4 já realiza purge automático; garantir que todos os templates estejam no `content` path.
...
1. Atualizar `tailwind.config.js` com os tokens.
